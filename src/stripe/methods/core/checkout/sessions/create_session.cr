...
class Stripe::Checkout::Session
  def self.create(
    ...
    {% for x in %w(mode payment_method_types cancel_url success_url client_reference_id customer customer_email line_items expand subscription_data allow_promotion_codes metadata) %}
